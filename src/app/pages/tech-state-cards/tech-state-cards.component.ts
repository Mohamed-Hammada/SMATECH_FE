...
import { CreateUpdateTechStateComponent } from '../tech-state/create-update-tech-state/create-update-tech-state.component';
import { TechStateCardDetailsComponent } from './tech-state-card-details/tech-state-card-details.component';

@Component({
  selector: 'app-tech-state-cards',
  templateUrl: './tech-state-cards.component.html',
  styleUrls: ['./tech-state-cards.component.css']
})
export class TechStateCardsComponent {
  ...
  selectedCardStatuses: CardStatus[] = [CardStatus.UNDER_REPAIR,CardStatus.UNDER_REPAIR_PENDING_ASSIGNMENT,CardStatus.WAITING_SPARE_PARTS,CardStatus.RETURN_NEEDS_FIX];
  ...
  onCardClick(row: any): void {
    ...
    const dialogRef = this.dialog.open(TechStateCardDetailsComponent, {
      width: '70%',
      data: { cardDetail: row }
    });
  }
